    ...
    <div class="">
                ...
                @foreach ($tags as $key => $tag) 
                    ...
                    <div class="px-2 font-bold text-gray-600">
                        <label
                            ...
                            <x-checkbox type="checkbox" wire:model.live="filters.tags" value="{{ $tag->id }}"></x-checkbox>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 items-stretch gap-8 py-8">
            @foreach ($articles as $article)
                <livewire:components.article-preview wire:key="{{ $article->id }}" :article="$article"></livewire:components.article-preview>
            @endforeach
        </div>

        @if ($empty_new_articles)
            <div class="h-48"><small>Il n'y a plus d'articles à afficher</small></div>
        @else
            <div id="scrollLoadingArticles" class="flex w-full justify-center items-center h-48">
                <div wire:loading class="h-16 w-16">

                    <div role="status">
                        <svg aria-hidden="true" class="text-primary-500/25 animate-spin fill-secondary-900"
